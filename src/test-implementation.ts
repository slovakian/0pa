...
// Test with Valibot
const valibotOp = opaWithContext.input(v.object({ name: v.string() })).handler(async ({ input, ctx }) => {
  return `Hello ${input.name} from ${ctx.db.user}`;
});

// Test with ArkType
const arktypeOp = opaWithContext.input(type({ name: 'string' })).handler(async ({ input, ctx }) => {
  return `Hello ${input.name} from ${ctx.db.user}`;
});

// Test with Zod
const zodOp = opaWithContext.input(z.object({ name: z.string() })).handler(async ({ input, ctx }) => {
  return `Hello ${input.name} from ${ctx.db.user}`;
});
...
const simpleOp = opaContextFree.input(v.object({ name: v.string() })).handler(async ({ input }) => {
  return `Hello ${input.name}`;
});

// Test standalone operations
const standaloneOp = op.input(v.object({ name: v.string() })).handler(async ({ input }) => {
  return `Hello ${input.name}`;
});
